...
static void	heredoc_in_child(int *pipe_fd, t_file *file, t_minishell **s)
{
	close(pipe_fd[0]);
...
int	handle_heredoc_execution(t_file *file, t_minishell **s)
{
	int		pipe_fd[2];
	pid_t	pid;
	int		status;
	...
	if (pid == 0)
		heredoc_in_child(pipe_fd, file, s);
	close(pipe_fd[1]);
...
int	handle_redirections(t_cmd *cmd, t_minishell **s)
{
	...
	while (tmp)
	{
		...
		{
			fd = handle_heredoc_execution(tmp, s);
			if (fd < 0 || dup2(fd, STDIN_FILENO) == -1)
